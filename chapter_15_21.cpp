...
public:
	...
	explicit Box(float width)
		:half_len_x(width*0.5f),half_len_y(width*0.5f),half_len_z(width*0.5f){}
	Box(float center_x,float center_y,float center_z,float len_x,float len_y,float len_z)
		:Shape_3D(center_x,center_y,center_x),half_len_x(len_x*0.5f),half_len_y(len_y*0.5f),half_len_z(len_z*0.5f){}

	inline virtual const char* shape_name()override { return "Box"; }
	inline virtual void resize_by_percentage(float pct)override
	{
		half_len_x *= pct; half_len_y *= pct; half_len_z *= pct;
	}
